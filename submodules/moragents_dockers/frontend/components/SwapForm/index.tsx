...
export const SwapForm: FC<SwapFormProps> = ({ isActive, onSubmitSwap, onSubmitApprove, onCancelSwap, fromMessage, selectedAgent }) => {
    ...
    const formatInterval = (intervalMs: number): { value: number, unit: string } => {
    ...
    const { value: intervalValue, unit: intervalUnit } = useMemo(() => formatInterval(fromMessage.interval), [fromMessage.interval]);
    ...
    const isSwapOrApproval = useMemo(() => {
        return (
            ...
            (allowance.data &&
                ...
                BigInt(parseFloat(String(formData.amount || 0)) * (10 ** parseFloat(String(decimals?.data || '18')))))
        );
    ...
    //                 allowance?.refetch();
    //             }, formData.interval || 5000); // Use the interval from formData
    ...
    // }, [isNativeToken, isSwapOrApproval, isActive, allowance, timeoutRefetchAllowance, formData.interval]);
                                ...
                                <Box flex="1" pr={2}>
                                    <VStack>
                                        ...
                                        <HStack>
                                            ...
                                            >{intervalValue.toFixed(2)}</Text>
                                            <Text>{intervalUnit}</Text>
                                        </HStack>
                                        <Text sx={{
                                            color: '#676B68',
                                            fontSize: '10px',
                                        }}>Powered by Biconomy</Text>
                                    </VStack>
                                </Box>
                                :
                                < Box flex="1" pr={2}>
